...
def combine(pcd1):
    for num in range(num_pcds-1):
        ...
        perc = 1 # 5
        batch_size = 200 # 500
        pts_to_sample = 200 # 5000

        # dataset specific parameters
        voxel_size = .001 # .01
        lrf_kernel = .3 * np.sqrt(3) # 3 * 3
        ...
        pcd2 = o3d.io.read_point_cloud('/pcd%d.ply' %(num+2))
        ...
        # write new PLY file
        pcd1_points = np.asarray(pcd1a.points)
        pcd1_color = np.asarray(pcd1a.colors)
        pcd2_points = np.asarray(pcd2a.points)
        pcd2_color = np.asarray(pcd2a.colors)
        ...
        pcd1 = pcd_combined
        pcd1 = pcd1.voxel_down_sample(0.0015)
    return pcd1


complete_pcd = combine(pcd1)
